...
# -----------------------------
st.set_page_config(
    page_title="QuantTrade Pro Terminal",
    layout="wide",
    initial_sidebar_state="expanded"
)

st.sidebar.title("🏦 QuantTrade Pro")
st.sidebar.markdown("Use the pages below to navigate between charting, AI predictions, and sentiment analysis.")
st.sidebar.markdown("---")
...
st.session_state["ticker_symbol"] = TICKERS[st.session_state["selected_company"]]
...
default_index = companies.index(st.session_state["selected_company"])

new_company = st.sidebar.selectbox("Select Asset", companies, index=default_index)
...
st.markdown(
    f"""
### Selected Asset: **{st.session_state['selected_company']}**
**Ticker:** `{st.session_state['ticker_symbol']}`

Use the pages from the sidebar:

- **📈 Market Summary** → price chart + RSI + MACD + Bollinger Bands
- **📰 Sentiment** → news headlines + sentiment view
- **🤖 AI Engine** → **next 30-day future price forecast**
"""
)